...
export default function Form({route}) {
  ...
  const [novoTitle, setNovoTitle] = useState(route.params?.title)
  const [novoDescription, setNovoDescription] = useState(route.params?.description)

  const navigation = useNavigation();

  const salvarTarefa = async () => {
    const body = JSON.stringify({title: novoTitle, description: novoDescription})
    if (id !== undefined){
  ...
  return (
    <View>
      ...
      onChangeText={(text)=> setNovoTitle(text)}
      placeholder="Título"
      />
      <TextInput
      style={styles.input}
      defaultValue={route.params?.description}
      onChangeText={(text)=> setNovoDescription(text)}
